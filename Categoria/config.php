...
class Config{
    ...
    public function __construct($id= 0, $nombres="", $decripcion="", $imagen= ""){
    ...
    public function insertData(){
        try {
            $stm =$this-> dbCnx -> prepare("INSERT INTO categoria(nombres, decripcion, imagen) values(?,?,?)");
    ...
    public function delete(){
        try {
            $stm =$this-> dbCnx -> prepare("DELETE FROM categoria WHERE id = ?");
            $stm->execute([$this->id]);
            return $stm-> fetchAll();
    ...
    public function selectOne(){
        try {
            $stm =$this-> dbCnx -> prepare("SELECT * FROM categoria WHERE id = ?");
            $stm->execute([$this->id]);
            return $stm-> fetchAll();
        } catch (Exception $error) {
            return $error->getMessage();
    ...
    public function update(){
        try {
            $stm =$this-> dbCnx -> prepare("UPDATE categoria SET nombres = ?, decripcion = ?, imagen = ? WHERE id = ?");
